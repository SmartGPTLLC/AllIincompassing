...
DECLARE
  ...
  v_locations jsonb;
BEGIN
  -- Therapists for dropdowns (fixed GROUP BY issue)
  SELECT jsonb_agg(t) INTO v_therapists
  FROM (
  ...
  -- Clients for dropdowns (fixed GROUP BY issue)
  ...
  -- Locations for dropdowns (fixed GROUP BY issue)
  SELECT jsonb_agg(l) INTO v_locations
  FROM (
